...
/**
 * This class utility class for creating connection to AWS Keyspaces for Cassandra.
 * It also has methods for creating the keyspace and table in AWS Keyspaces
 */
public class DatabaseUtility {
    ...
    static {
        ...
        KEYSTORE_FILE_TYPE = PropertyConfig.getProperty("ssl.keyStoreFile.type");
        KEYSTORE_FILE_PATH = PropertyConfig.getProperty("ssl.keyStoreFile.path");
        KEYSTORE_FILE_PASSWORD = PropertyConfig.getProperty("ssl.keyStoreFile.password");
        KEYSPACE = PropertyConfig.getProperty("keyspace.name");
        TABLE_NAME = PropertyConfig.getProperty("table.name");
    }

    /* **TODO**
     * The below method should create a CQLSession with AWS keyspaces and return the session
     ...
     * SingleRegionStrategy should be used for replication
     */
    public void createKeyspace(CqlSession session) {
        CreateKeyspace newKeyspace = SchemaBuilder.createKeyspace(KEYSPACE)
                .ifNotExists()
                .withReplicationOptions(ImmutableMap.of("class", (Object) "SingleRegionStrategy"));
        session.execute(newKeyspace.build().setTimeout(Duration.ofSeconds(15)));
    }
